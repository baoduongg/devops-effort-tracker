...
description: Writes resilient end-to-end tests (Playwright/Cypress style) with stable selectors, isolated test data, and flake elimination, and debugs failing test runs via traces. Auto-invoke after a feature is implemented and reviewed, when test coverage is needed, or when CI tests are flaky.
