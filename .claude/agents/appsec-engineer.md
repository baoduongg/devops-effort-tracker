...
description: Performs threat modeling and secure code review, and integrates SAST/DAST checks to catch vulnerabilities before production. Auto-invoke when a change touches auth, permissions, user input handling, secrets, or other security-sensitive surfaces, or when the user asks for a security review.
